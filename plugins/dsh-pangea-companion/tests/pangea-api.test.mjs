...
test('creates one managed pending Skill request and removes it after Run creation', async () => {
  ...
  try {
    ...
    assert.deepEqual(observed.contract.asset_ids, ['asset-not-in-current-contract'])
    assert.deepEqual(observed.contract.test_case_examples, ['TC-not-in-current-contract'])

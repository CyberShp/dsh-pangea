      ...
      return h('div', { style: styles.root, role: 'region', 'aria-label': 'PANGEA 资产管理' },
        ...
        h('div', { style: styles.content },
          ...
          showLibrary && !activeAsset ? h('div', { style: { ...styles.wrap, marginBottom: 10 } },
            h('select', { 'aria-label': '筛选资产类型', style: styles.input, value: type, onChange: event => { setPage(1); setKind(''); setType(event.target.value) } }, TYPES.filter(([value]) => state?.features?.revisions !== false || value !== 'test_case_example').map(([value, label]) => h('option', { key: value || 'all', value }, value ? label : '全部类型'))),

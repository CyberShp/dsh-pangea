      ...
      return h('div', { style: styles.root, role: 'region', 'aria-label': 'PANGEA 资产管理' },
        ...
        h('div', { style: styles.content },
          ...
          importOpen ? h('div', { style: styles.card },
            ...
            h('div', { style: styles.meta }, '选择文件和类型，预览后导入。遇到已有内容时可选择复用，或将同名文件更新为新修订。'),
          ...
          showLibrary && !activeAsset ? h('div', { style: { ...styles.wrap, marginBottom: 10 } },
            h('select', { 'aria-label': '筛选资产类型', style: styles.input, value: type, onChange: event => { setPage(1); setKind(''); setType(event.target.value) } }, TYPES.map(([value, label]) => h('option', { key: value || 'all', value }, value ? label : '全部类型'))),
